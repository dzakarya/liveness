...
task_list = ["Give Me Your Best Smile","Tilt Your Head Left","Tilt Your Head Rigth","Open Your Mouth","Close One of Your Eyes"]
status = ""
task_idx = random.randint(0,len(task_list)-1)
while True:
    ...
    for rect in rects:
        ...
        if headDeg < -hdth:
            head_status = "tilt_rigth"
        elif headDeg > hdth:
            head_status = "tilt_left"
        else:
            head_status = "normal"
        if task_idx == 0:
            if mouth_status == "smiling_face":
                status = "Pass"
            else:
                status = "Reject"
        elif task_idx == 1:
            if head_status == "tilt_left":
                status = "Pass"
            else:
                status = "Reject"
        elif task_idx == 2:
            if head_status == "tilt_rigth":
                status = "Pass"
            else:
                status = "Reject"
        elif task_idx == 3:
            ...
            if left_eye_status == "closed" or rigth_eye_status == "closed":
                status = "Pass"
            else:
                status = "Reject"
        ...
        cv2.putText(frame, "Task: {}".format(task_list[task_idx]), (0, 20),
            cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255,0), 2)
        cv2.putText(frame, "status: {}".format(status), (0, 70),
            cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255,0), 2)
    # show the frame
